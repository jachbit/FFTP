...
def build():
    ...
    save_data = [
        ...
        [Paragraph("Auto-Save", TD_B),
         Paragraph("Always ON — happens automatically on every change, no button needed", TD_L),
         Paragraph("Green dot badge at bottom-right confirms it is active. Shows how many log rows are saved.", TD_L)],
        [Paragraph("Clear Data", TD_B),
         Paragraph("Click the red Clear button in the auto-save badge", TD_L),
         Paragraph("A confirmation modal appears listing exactly what will be deleted. You must click 'Yes, Clear Everything' to confirm — Cancel keeps all data safe. Export CSV first if you need a backup.", TD_L)],
    ...
    story += [sp(8),
        ...
        bullet("<b>Never click Clear in the auto-save badge unless you intend to reset everything.</b> A confirmation modal will warn you first — read it carefully."),
        bullet("<b>Export CSV regularly</b> to keep a backup before clearing or switching devices."),
